...
function selectContact(id,name,canMessage=true){contactId=id;groupId=null;lastId=0;chatTitle.textContent=name;chatArea.innerHTML='';chatForm.style.display='flex';chatHeaderAvatar.style.visibility='visible';chatHeaderAvatar.innerText=name&&name.length?name.charAt(0).toUpperCase():'?';chatHeaderAvatar.style.backgroundColor=id? 'var(--primary-color)' : '#6366f1';if(canMessage){messageInput.disabled=false;messageInput.placeholder="Type a message"}else{messageInput.disabled=true;messageInput.placeholder="this user is set the account on private"}if(apiLeaveGroup)apiLeaveGroup.style.display='none';if(window.innerWidth<640){document.body.classList.add('in-chat')}loadUsers()}
function selectGroup(gid,name){groupId=gid;contactId=null;lastId=0;chatTitle.textContent=name;chatArea.innerHTML='';chatForm.style.display='flex';chatHeaderAvatar.style.visibility='visible';chatHeaderAvatar.innerText=name&&name.length?name.charAt(0).toUpperCase():'G';chatHeaderAvatar.style.backgroundColor='#1e293b';messageInput.disabled=false;messageInput.placeholder="Type a message";if(apiLeaveGroup)apiLeaveGroup.style.display='inline-flex';if(window.innerWidth<640){document.body.classList.add('in-chat')}loadUsers()}
